...
A voice-driven role-playing game built with LiveKit agents, featuring dynamic narrator and combat agents, D&D-style mechanics, and persistent character progression.

## Overview

Agents and Storms demonstrates how to build an engaging voice-driven game using LiveKit's agent framework. The game features:
...
- **Trading System**: Trade items and gold with NPCs using charisma-based negotiations
- **Dynamic NPC Inventories**: NPCs have contextually appropriate items generated by AI based on their role and location
- **Loot System**: Defeated enemies drop their inventory and gold
...
1. **character.py** - Character classes and progression system
   - `Character`: Base class with stats and combat mechanics
   - `PlayerCharacter`: Extended with inventory and progression
   - `NPCCharacter`: AI-controlled characters with behaviors

2. **game_mechanics.py** - Game rules and systems
   - `DiceRoller`: Handles all dice rolling
   - `Combat`: Turn-based combat system
   - `SkillCheck`: Ability and skill checks
   - `SpellCasting`: Magic system

3. **agent.py** - Agent implementations
   - `NarratorAgent`: Storytelling and exploration
   - `CombatAgent`: Fast-paced combat management
   - `GameUserData`: Persistent game state
